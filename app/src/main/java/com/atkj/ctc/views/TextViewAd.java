...
import com.atkj.ctc.bean.ADEnity;
...
public class TextViewAd extends android.support.v7.widget.AppCompatTextView {
    ...
    private List<NoticeBean.ObjBean> mTexts; //显示文字的数据源
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if (onClickLitener != null) {
                    onClickLitener.onClick(mTexts.get(mIndex).getPath());
                }
    ...
    //设置数据源
    public void setmTexts(List<NoticeBean.ObjBean> mTexts) {
        this.mTexts = mTexts;
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mTexts != null) {
            Log.i(TAG, "onDraw: " + mY);
            NoticeBean.ObjBean model = mTexts.get(mIndex);
            String font = model.getContent();
            String back = model.getContent();
            //绘制前缀
